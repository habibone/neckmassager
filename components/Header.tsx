...
const Header: React.FC<HeaderProps> = ({ onCtaClick }) => {
  ...
  return (
    <div className="w-full">
      <div className="bg-[#6149dd] text-white py-2 px-4 text-center text-[10px] md:text-xs font-black uppercase tracking-widest overflow-hidden">
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-100 px-6 py-3">
        <div className="max-w-screen-xl mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <div className="w-9 h-9 gradient-cta rounded-xl flex items-center justify-center shadow-lg shadow-[#6149dd]/20">
              <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
            </div>
            <span className="text-xl font-black tracking-tighter text-[#111827]">VIBE<span className="text-[#6149dd]">SLIM</span></span>
          </div>
          
          <div className="flex items-center space-x-4 md:space-x-6">
            <nav className="hidden md:flex space-x-8 text-[10px] font-black text-[#848693] uppercase tracking-widest">
              {navLinks.map((link) => (
                <a 
                  key={link.id}
                  href={`#${link.id}`} 
                  onClick={(e) => scrollToSection(e, link.id)}
                  className="hover:text-[#6149dd] transition-colors"
                >
                  {link.name}
                </a>
              ))}
            ...
            <button 
              ...
              className="md:hidden p-2 text-[#848693] hover:text-[#6149dd] transition-colors"
        ...
        <div className={`fixed inset-0 top-[100px] z-40 bg-white/95 backdrop-blur-xl md:hidden transition-all duration-300 ease-in-out ${isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'}`}>
          <nav className="flex flex-col items-center justify-center space-y-8 p-12 text-2xl font-black text-gray-900 uppercase tracking-tighter">
            ...
            <div className="w-12 h-1 bg-[#6149dd]/10 rounded-full"></div>
            <button 
              onClick={onCtaClick}
